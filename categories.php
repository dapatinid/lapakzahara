<?php 
   ...
   // Ambil slug kategori dari URL (misal: categories/nama-kategori)
   $uriSegments = explode('/', parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
   $slug = end($uriSegments);
   
   // Gunakan slug dalam query untuk mendapatkan informasi kategori
   $kategoriQuery = $server->query("SELECT * FROM `kategori` WHERE `slug`='$slug'");
   $kategori_data = mysqli_fetch_assoc($kategoriQuery);
...
<html lang="en">
   <head>
      ...
      <title>Jual Produk <?php echo $kategori_data['nama']; ?> Terbaru & Terlengkap | <?php echo $title_name; ?></title>
      <!-- META SEO -->
   ...
   <body>
      ...
      <div class="width">
         ...
         <div class="box_produk">
            <?php
               if ($kategori_data) {
                   $kategori_id = $kategori_data['id']; // Dapatkan ID kategori dari data yang ditemukan
               
                   // Gunakan ID kategori dalam query untuk mendapatkan produk berdasarkan kategori
                   ...
                   WHERE iklan.id_kategori='$kategori_id' AND iklan.status_moderasi = 'Diterima'
