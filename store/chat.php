...
<html lang="en">
   <head>
      ...
      <title>Chatting Toko <?php echo $profile['nama_lengkap']; ?> | <?php echo $title_name; ?></title>
      <!-- META SEO -->
                        ...
                        <div class="text_list_chat">
                           <div class="name_text_list_chat">
                              <div class="isi_name_text_list_chat">
                                 <p>
                                    <?php echo $data_user_chat['nama_lengkap']; ?>
                                    <?php if ($data_user_chat['verifikasi_user'] == 'Ya') { ?>
                                    <img src="<?php echo $url; ?>/assets/icons/verifikasi-user.png" id="img-centang-biru" data-original-title="Centang Biru" class="has-tooltip" style="width: 20px; height: auto; vertical-align: middle;" onclick="handleCentangBiru('<?php echo $valueiduserarr; ?>');">
                                    <?php } ?>
                                 </p>
                              </div>
                              <div class="isi_time_text_list_chat">
                                 <?php if ($tanggal_sekarang == $tanggal_chat_view) { ?>
                           ...
                           <div class="box_isi_text_chat">
                              <?php if ($data_chat_view['pengirim_user_id'] != $valueiduserarr) { ?>
                              <?php if ($data_chat_view['status'] == '') { ?>
      ...
      <script>
         window.onload = function() {
             ...
             if (width_co < 900) {
                 ...
                 if (list_chat && detail_chat) {
                     list_chat.style.display = 'none';
                     detail_chat.style.display = 'flex';
                 }
             }
         
             // Assuming show_detail_chat is a function defined elsewhere
             show_detail_chat('<?php echo $data_s_userchat['id']; ?>', '../assets/image/profile/<?php echo $data_s_userchat['foto']; ?>', '<?php echo $data_s_userchat['nama_lengkap']; ?> <?php if ($data_s_userchat['verifikasi_user'] == 'Ya') { echo '<img src="../assets/icons/verifikasi-user.png" id="img-verif" data-original-title="null" class="has-tooltip" style="width: 20px;height: auto;vertical-align: middle;">'; } ?>');
         }
      </script> 
      <?php
         }
