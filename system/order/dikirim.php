...
$result = $server->query("SELECT * FROM `invoice`, `iklan`, `kategori` WHERE invoice.id_user='$iduser' AND invoice.tipe_progress='Dikirim' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id ORDER BY `invoice`.`idinvoice` DESC ");
$total = mysqli_num_rows($result);

$select_invoice = $server->query("SELECT * FROM `invoice`, `iklan`, `kategori` WHERE invoice.id_user='$iduser' AND invoice.tipe_progress='Dikirim' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id ORDER BY `invoice`.`idinvoice` DESC LIMIT $mulai, $halaman ");
        ...
        ?>
            ...
            <a href="<?php echo $url; ?>/checkout/detail/<?php echo $invoice_data['idinvoice']; ?>">
                ...
                <div class="box_detail_isi_cart">
                    ...
                    <a href="<?php echo $url; ?>/resi/hasil.php?waybill=<?php echo $invoice_data['resi']; ?>&courier=<?php echo $invoice_data['kurir_manual']; ?>&submit=Cek+Resi">
                    <div class="bayar" id="button_checkout44" style="width: auto; padding: 0 15px;"><i class="fa fa-truck"></i></div>
        </a>
                </div>
            </div>
        <?php
