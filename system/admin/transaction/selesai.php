...
if ($tipe_user_vt == 'store') {
    $result = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Selesai' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id AND iklan.user_id='$iduser' ORDER BY `invoice`.`idinvoice` DESC ");
    $select_invoice = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Selesai' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id AND iklan.user_id='$iduser' ORDER BY `invoice`.`idinvoice` DESC LIMIT $mulai, $halaman ");
} else {
    $result = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Selesai' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id ORDER BY `invoice`.`idinvoice` DESC ");
    $select_invoice = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Selesai' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id ORDER BY `invoice`.`idinvoice` DESC LIMIT $mulai, $halaman ");
}
...
?>
    <script>
        load_paging_selesai_id.style.display = 'none';
    </script>
<?php
} else {
?>
    <script>
        load_paging_selesai_id.style.display = 'block';
    </script>
<?php
}
if ($total == "0") {
