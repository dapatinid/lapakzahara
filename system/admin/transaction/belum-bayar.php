...
if ($tipe_user_vt == 'store') {
    $result = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Belum Bayar' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id AND iklan.user_id='$iduser' ORDER BY `invoice`.`idinvoice` DESC ");
    $select_invoice = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Belum Bayar' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id AND iklan.user_id='$iduser' ORDER BY `invoice`.`idinvoice` DESC LIMIT $mulai, $halaman ");
} else {
    $result = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Belum Bayar' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id ORDER BY `invoice`.`idinvoice` DESC ");
    $select_invoice = $server->query("SELECT * FROM `iklan`, `kategori`, `akun`, `invoice` WHERE  invoice.tipe_progress='Belum Bayar' AND invoice.id_iklan=iklan.id AND iklan.id_kategori=kategori.id AND invoice.id_user=akun.id ORDER BY `invoice`.`idinvoice` DESC LIMIT $mulai, $halaman ");
}
...
?>
    <div class="box_isi_res_order">
        ...
        ?>
            <div class="isi_cart" id="isi_cart_hpp<?php echo $invoice_data['idinvoice']; ?>">
                ...
                <div class="box_detail_isi_cart">
                    ...
                    if ($invoice_data['bank_manual'] || $invoice_data['bukti_transfer']) {
                    ?>
                        <div class="konfirmasi" onclick="step_paket_konfirmasi('<?php echo $invoice_data['idinvoice']; ?>', '<?php echo $invoice_data['id_user']; ?>')">
                    ...
                    <div class="hapus" onclick="hapus_pesanan('<?php echo $invoice_data['idinvoice']; ?>')">
                    <i class="ri-delete-bin-line"></i>
                        <img src="<?php echo $url; ?>/assets/icons/loading-w.svg" id="load_hpp<?php echo $invoice_data['idinvoice']; ?>">
                    </div>
                </div>
            </div>
        <?php
