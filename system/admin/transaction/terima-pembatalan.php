...
$idinvoicesb = $_POST['idinvoice_hp'];
...
$update_pembatalan = $server->prepare("UPDATE `pembatalan_transaksi` SET `status_persetujuan`='Disetujui' WHERE `id_invoice`='$idinvoicesb'");
$update_pembatalan->execute(); // Tambahkan eksekusi update

// Update status invoice
$update_dikirim = $server->prepare("UPDATE `invoice` SET `tipe_progress`='Dibatalkan', `waktu_dibatalkan`='$time' WHERE `idinvoice`='$idinvoicesb'");
$update_dikirim->execute(); // Tambahkan eksekusi update
...
$insert_notif_ud = $server->query("INSERT INTO `notification`(`id_user`, `id_invoice`, `nama_notif`, `deskripsi_notif`, `waktu_notif`, `status_notif`) 
    VALUES ('$iduser_ud', '$idinvoicesb', 'Pesanan Telah Dibatalkan', 
    'Pesanan Anda telah berhasil dibatalkan. Dana pengembalian sebesar Rp" . number_format($jumlah_pengembalian, 0, ',', '.') . " untuk produk $judul_barang_mp telah masuk ke saldo Anda! Belanja lagi yuk atau cek produk lain yang bisa jadi favorit baru.', 
    '$time', '')");
