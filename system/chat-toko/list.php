...
function makeLinksClickable($text) {
    // The regular expression pattern to match URLs
    $pattern = '/(http[s]?:\/\/[^\s]+)/';
...
$update_dibaca_chat = $server->query("UPDATE `chat_toko` SET `status`='dibaca' WHERE `pengirim_user_id`='$v_id_chat' AND `penerima_user_id`='$iduser' ");

$select_chat_view = $server->query("SELECT * FROM `chat_toko` WHERE `pengirim_user_id`='$iduser' AND `penerima_user_id`='$v_id_chat' OR `pengirim_user_id`='$v_id_chat' AND `penerima_user_id`='$iduser' ORDER BY `chat_toko`.`id` ASC ");
while ($data_chat_view = mysqli_fetch_assoc($select_chat_view)) {
    ...
    $text_chat_view = makeLinksClickable($data_chat_view['text_chat']);
       ...
       <div class="box_isi_chat">
            <div class="box_bubble_chat_left">
                <div class="bubble_chat_left">
                    <p class="isi_bubble_chat_left"><?php echo $text_chat_view; ?></p>
                </div>
        ...
        <div class="box_isi_chat">
            <div class="box_bubble_chat_right">
                <div class="bubble_chat_right">
                    <p class="isi_bubble_chat_right"><?php echo $text_chat_view; ?></p>
                </div>
