<?php 
   ...
   // Ambil slug brand dari URL (misal: categories/nama-brand)
   $uriSegments = explode('/', parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH));
   $slug = end($uriSegments);
   
   // Gunakan slug dalam query untuk mendapatkan informasi brand
   $brandQuery = $server->query("SELECT * FROM `brand` WHERE `slug`='$slug'");
   $brand_data = mysqli_fetch_assoc($brandQuery);
...
<html lang="en">
   <head>
      ...
      <title>Jual Produk <?php echo $brand_data['namab']; ?> Terbaru & Terlengkap | <?php echo $title_name; ?></title>
      <!-- META SEO -->
   ...
   <body>
      ...
      <div class="width">
   <div class="box_judul">
      <p>Brand <span><?php echo $brand_data['namab']; ?></span></p>
   </div>
   <div class="box_produk">
      <?php
         if ($brand_data) {
             $brand_id = $brand_data['id']; // Dapatkan ID brand dari data yang ditemukan

             // Gunakan ID brand dalam query untuk mendapatkan produk berdasarkan brand
             $produk = $server->query("SELECT iklan.*, brand.namab AS brand_namab, akun.nama_pengguna, akun.verifikasi_toko 
             FROM `iklan` 
             LEFT JOIN `brand` ON iklan.id_brand=brand.id 
             LEFT JOIN `akun` ON iklan.user_id=akun.id  
             WHERE iklan.id_brand='$brand_id' AND iklan.status_moderasi = 'Diterima'
...
$after_dot_rat = substr($data_rata_rating_vp['rata_rat'], 2, 1);
                 ...
                 if ($produk_data['status'] == '' && ($produk_data['stok'] !== $produk_data['terjual'] or $produk_data['stok'] > $produk_data['terjual'])) {
